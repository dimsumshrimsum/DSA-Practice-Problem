...
function index(data) {
  //initialiaze empty object
  // for each (pass index)line split each line using the regex, comms symbols spaces not hyphens
  // for each line, toLowerCase().split(regex).filter(Boolean)
  // forEach my new array
  // check if key exists in object if exissts add index to set set.add(index)
  //if doenst exist add key to object and create new set([index])
  const result = {};
